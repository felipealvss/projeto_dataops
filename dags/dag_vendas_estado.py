...
POSTGRES_TABLE = "vendas_por_cidade"  # Novo nome da tabela

# Função para consultar dados do MongoDB e agrupar por cidade e estado
def consultar_e_agrupar_vendas_por_cidade(**context):
    try:
        # Conectar ao MongoDB
        ...
        if resultado:
            ...
            df['cidade'] = df['localizacao'].apply(lambda x: x['cidade'])
            df['estado'] = df['localizacao'].apply(lambda x: x['estado'])
...
def inserir_dados_no_postgres(df, **context):
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS vendas_por_cidade (
                cidade VARCHAR(255),
                estado VARCHAR(255),
                total_vendas INT,
        ...
        for _, row in df.iterrows():
            cursor.execute("""
                INSERT INTO vendas_por_cidade (cidade, estado, total_vendas, total_faturado, quantidade_motos_vendidas)
                VALUES (%s, %s, %s, %s, %s)
                """, (row['cidade'], row['estado'], row['total_vendas'], row['total_faturado'], row['quantidade_motos_vendidas']))
...
# Tarefa para consultar e agrupar dados do MongoDB por cidade e estado
consultar_e_agrupar_task = PythonOperator(
    task_id="consultar_e_agrupar_vendas_por_cidade",
    python_callable=consultar_e_agrupar_vendas_por_cidade,
...
inserir_dados_task = PythonOperator(
    ...
    provide_context=True,
    op_args=['{{ task_instance.xcom_pull(task_ids="consultar_e_agrupar_vendas_por_cidade") }}'],  # Passa os dados para a próxima tarefa
    dag=dag
)
